...
class ProxyWebServer(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path.startswith('/api/'):
            ...
            # React 라우팅을 위한 처리
            if not os.path.exists(file_path) and not file_path.startswith('api/'):
                file_path = 'index.html'
            ...
            try:
                ...
                # 파일이 없으면 index.html로 리다이렉트 (React Router)
                try:
...
if __name__ == '__main__':
    ...
    if not create_self_signed_cert():
        ...
        PORT = 80
        try:
            with socketserver.TCPServer(("", PORT), ProxyWebServer) as httpd:
                print(f"프록시 서버가 포트 {PORT}에서 실행 중입니다")
                print(f"React 프론트엔드 + 스프링부트 백엔드 연결됨")
        ...
        try:
            with socketserver.TCPServer(("", PORT), ProxyWebServer) as httpd:
                ...
                print(f"HTTPS 프록시 서버가 포트 {PORT}에서 실행 중입니다")
                print(f"React 프론트엔드 + 스프링부트 백엔드 연결됨")
